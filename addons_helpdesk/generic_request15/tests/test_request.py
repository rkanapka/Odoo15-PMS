...
class TestRequestBase(RequestCase):
    ...
    def test_180_request_close_via_wizard(self):
        request = self.env.ref(
            'generic_request15.request_request_type_sequence_demo_1')
        request.stage_id = self.env.ref(
            'generic_request15.request_stage_type_sequence_sent')

        close_stage = self.env.ref(
            'generic_request15.request_stage_type_sequence_closed')
        close_route = self.env.ref(
            'generic_request15.request_stage_route_type_sequence_sent_to_closed')
    ...
    def test_240_request_events(self):
        ...
        with freeze_time('2018-07-25'):
            ...
            cron_job = self.env.ref(
                'generic_request15.ir_cron_request_vacuum_events')
    ...
    def test_250_request_create_simple_without_channel(self):
        Request = self.env['request.request']
        channel_other = self.env.ref('generic_request15.request_channel_other')
    ...
    def test_260_request_create_simple_with_channel_call(self):
        Request = self.env['request.request']
        channel_call = self.env.ref('generic_request15.request_channel_call')
    ...
    def test_unlink_just_created(self):
        ...
        self.request_manager.groups_id |= self.env.ref(
            'generic_request15.group_request_manager_can_delete_request')
        request.unlink()
    ...
    def test_request_suggested_recipients(self):
        ...
        request = self.env.ref(
            'generic_request15.request_request_type_simple_demo_3')
    ...
    def test_default_response_text(self):
        ...
        close_route = self.env.ref(
            'generic_request15.request_stage_route_type_access_sent_to_rejected')
        close_stage = self.env.ref(
            'generic_request15.request_stage_route_type_access_sent_to_rejected')
    ...
    def test_complex_priority(self):
        test_request = self.env.ref(
            'generic_request15.demo_request_with_complex_priority')
