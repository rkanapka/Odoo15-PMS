...
from odoo.addons.generic_mixin15.tests.common import (
    ReduceLoggingMixin,
...
def disable_mail_auto_delete(env):
    ...
    env['mail.mail']._patch_method('create', patched_method)
...
class RequestCase(AccessRulesFixMixinST,
    ...
    def setUpClass(cls):
        ...
        cls.resource_category = cls.env.ref(
            'generic_request15.request_category_demo_resource')
        cls.tec_configuration_category = cls.env.ref(
            'generic_request15.request_category_demo_technical_configuration')

        # Request type
        cls.simple_type = cls.env.ref('generic_request15.request_type_simple')
        cls.sequence_type = cls.env.ref(
            'generic_request15.request_type_sequence')
        cls.non_ascii_type = cls.env.ref(
            'generic_request15.request_type_non_ascii')
        cls.access_type = cls.env.ref(
            'generic_request15.request_type_access')

        # Stages
        cls.stage_draft = cls.env.ref(
            'generic_request15.request_stage_type_simple_draft')
        cls.stage_sent = cls.env.ref(
            'generic_request15.request_stage_type_simple_sent')
        cls.stage_confirmed = cls.env.ref(
            'generic_request15.request_stage_type_simple_confirmed')
        cls.stage_rejected = cls.env.ref(
            'generic_request15.request_stage_type_simple_rejected')
        cls.stage_new = cls.env.ref(
            'generic_request15.request_stage_type_sequence_new')
        # Routes
        cls.route_draft_to_sent = cls.env.ref(
            'generic_request15.request_stage_route_type_simple_draft_to_sent')
        cls.non_ascii_route_draft_to_sent = cls.env.ref(
            'generic_request15.request_stage_route_type_non_ascii_draft_to_sent')

        # Requests
        cls.request_1 = cls.env.ref(
            'generic_request15.request_request_type_simple_demo_1')
        cls.request_2 = cls.env.ref(
            'generic_request15.request_request_type_access_demo_1')

        # Users
        cls.demo_user = cls.env.ref('base.user_demo')
        cls.request_user = cls.env.ref(
            'generic_request15.user_demo_request')
        cls.request_manager = cls.env.ref(
            'generic_request15.user_demo_request_manager')
        cls.request_manager_2 = cls.env.ref(
            'generic_request15.user_demo_request_manager_2')

        # Creation template
        cls.creation_template = cls.env.ref(
            'generic_request15.demo_request_creation_template')

        # Request kind
        cls.request_kind = cls.env.ref('generic_request15.request_kind_demo')
...
class AccessRightsCase(AccessRulesFixMixinST,
    ...
    def setUpClass(cls):
        ...
        # Users
        cls.demo_user = cls.env.ref('generic_request15.user_demo_request')
        ...
        # Request Type
        cls.usimple_type = cls.uenv.ref('generic_request15.request_type_simple')
        cls.msimple_type = cls.menv.ref('generic_request15.request_type_simple')
