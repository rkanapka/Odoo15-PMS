...
from odoo.addons.generic_request15.tests.common import disable_mail_auto_delete
from odoo.addons.generic_mixin15.tests.common import TEST_URL
...
class TestRequestPortalMail(TestPhantomTour):
    ...
    def setUp(self):
        ...
        self.wsd_user = self.env.ref('crnd_wsd15.user_demo_service_desk_website')
    ...
    def test_assign_portal(self):
        request = self.env['request.request'].with_context(
            ...
            'type_id': self.env.ref('generic_request15.request_type_simple').id,
            'request_text': 'Test',
        })
    ...
    def test_view_on_website_button(self):
        request = self.env.ref(
            'crnd_wsd15.demo_request_request_generic_question_1')
