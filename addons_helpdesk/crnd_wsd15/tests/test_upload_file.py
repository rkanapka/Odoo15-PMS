...
class TestUploadFile(TestPhantomTour):
    def setUp(self):
        ...
        self.user = self.env.ref('crnd_wsd15.user_demo_service_desk_website')
        self.request_type = self.env.ref('crnd_service_desk15.request_type_incident')
    ...
    def test_upload_file_existing_request(self):
        ...
        # test upload file
        file_name = get_module_resource('crnd_wsd15', 'static/description', 'index.html')
        files = {
        ...
        file_name = get_module_resource('crnd_wsd15', 'static/description', 'banner.gif')
        files = {
            'upload': open(file_name, 'rb'),
        }
